...
export class NotificationsComponent implements OnInit {
  ...
  GetNotifications() {
    ...
    this.uamNewService.GetNotifications().subscribe(data => {
      ...
      // //this.notifications = [];
      if (data) {
        this.appUpdates = _.filter(data, { title: this.authorizationService.config[0].announcement_notification_type });
        this.notifications = _.reject(data, { title: this.authorizationService.config[0].announcement_notification_type });
      ...
      for (var notInd = 0; notInd < this.notifications.length; notInd++) {
        ...
        if (curNotification && curNotification.isCompleted == 0 && moment(curNotification.toDate) <= moment()) { //.format('DD-MMM-YYYY HH:mm AM') .format('DD-MMM-YYYY HH:mm AM')
